...
static int adios_init_sched(struct request_queue *q, struct elevator_type *e) {
	...
	ad->optimize_spatial = is_rotational =
		q->limits.features & BLK_FEAT_ROTATIONAL;

	ad->global_latency_window = is_rotational ?
		default_global_latency_window_r :
...
// New sysfs attribute for optimize_spatial
static ssize_t adios_optimize_spatial_store(
		struct elevator_queue *e, const char *page, size_t count) {
	struct adios_data *ad = e->elevator_data;
	unsigned long val;
	int ret;
	...
	ad->optimize_spatial = !!val;

	return count;
}

static ssize_t adios_optimize_spatial_show(
		struct elevator_queue *e, char *page) {
	struct adios_data *ad = e->elevator_data;
	return sprintf(page, "%d\n", ad->optimize_spatial);
}

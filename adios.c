...
static u64 latency_target[ADIOS_OPTYPES] = {
...
static unsigned int batch_limit[ADIOS_OPTYPES] = {
	[ADIOS_READ]    = 16,
	[ADIOS_WRITE]   =  8,
	[ADIOS_DISCARD] =  1,
	[ADIOS_OTHER]   =  1,
...
static void add_to_dl_tree(struct adios_data *ad, struct request *rq) {
	...
	rd->deadline =
		rq->start_time_ns + latency_target[optype] + rd->pred_lat;
...
static bool fill_batch_queues(struct adios_data *ad, u64 current_lat) {
	...
	while (true) {
		...
		if (count && (!ad->latency_model[optype].base || 
			ad->batch_count[page][optype] >= batch_limit[optype] ||
